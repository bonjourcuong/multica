...
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, scope, user_id
`
...
func (q *Queries) CreateGlobalAgent(ctx context.Context, arg CreateGlobalAgentParams) (Agent, error) {
...
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, scope, user_id FROM agent
WHERE id = $1
`
...
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, last_heartbeat_at, global_session_id FROM agent_task_queue
WHERE id = $1
`
...
const getGlobalAgentByUser = `-- name: GetGlobalAgentByUser :one
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, scope, user_id FROM agent
...
func (q *Queries) GetGlobalAgentByUser(ctx context.Context, userID pgtype.UUID) (Agent, error) {
	row := q.db.QueryRow(ctx, getGlobalAgentByUser, userID)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const getGlobalAgentByUserAndName = `-- name: GetGlobalAgentByUserAndName :one
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, scope, user_id FROM agent
WHERE scope = 'global' AND user_id = $1 AND name = $2
LIMIT 1
`

type GetGlobalAgentByUserAndNameParams struct {
	UserID pgtype.UUID `json:"user_id"`
	Name   string      `json:"name"`
}
...
func (q *Queries) GetGlobalAgentByUserAndName(ctx context.Context, arg GetGlobalAgentByUserAndNameParams) (Agent, error) {
	row := q.db.QueryRow(ctx, getGlobalAgentByUserAndName, arg.UserID, arg.Name)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const listGlobalAgentsByUser = `-- name: ListGlobalAgentsByUser :many
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, scope, user_id FROM agent
...
func (q *Queries) ListGlobalAgentsByUser(ctx context.Context, userID pgtype.UUID) ([]Agent, error) {
	rows, err := q.db.Query(ctx, listGlobalAgentsByUser, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
