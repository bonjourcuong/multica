...
// The display text supports one level of nested brackets so agent labels like
// "QA Bruce [MF]" parse correctly: `(?:[^\[\]]|\[[^\[\]]*\])*` accepts any non-bracket
// run, optionally interleaved with a single-level `[...]` group.
